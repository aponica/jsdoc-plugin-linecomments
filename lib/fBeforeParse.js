//=============================================================================
//  Copyright 2020-2021 Opplaud LLC and other contributors. MIT licensed.
//=============================================================================
  ...
  iEvent.source =
    iEvent.source.split( '\n' ).map( zLine => {
      ...
      if ( null === zStart ) { // look for start
        ...
        if ( null === reReplace ) { // find replacement

          const azMatches = zLine.match( /^\s*\/\/(\s*)/ );

          if ( null === azMatches )
            throw new Error( iEvent.filename + ': line ' + nLine +
              ': missing // (or maybe an earlier doc comment was not ' +
              'terminated correctly!)' );

          reReplace = new RegExp( '^\\s*\/\/(' + azMatches[ 1 ] + ')?' );
